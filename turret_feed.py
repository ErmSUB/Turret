"""
turret_feed.py - USB webcam + YOLO + motors + live annotated stream.

Run on the Pi:
  python3 turret_feed.py
...
# Step mode: short pulse then pause for next frame
STEP_DURATION = 0.05
...
def draw(frame, detections, primary, tx, ty, fps, locked):
    ...
    if primary:
        cv2.rectangle(
            frame,
            (primary["x1"], primary["y1"]),
            (primary["x2"], primary["y2"]),
            (0, 0, 220),
            3,
        )
        text(
            frame,
            f"FOCUS {primary['conf'] * 100:.0f}%",
            (primary["x1"], max(primary["y1"] - 6, 14)),
            0.55,
            (0, 0, 220),
            2,
        )
        ...
        while True:
            ...
            if detections:
                if tracked is None:
                    ...
                    best = min(
                        detections,
                        key=lambda d: ((d["x1"] + d["x2"]) // 2 - tcx) ** 2
                        + ((d["y1"] + d["y2"]) // 2 - tcy) ** 2,
                    )
                ...
                if tracked is None:
                    ...
                    for i, value in enumerate([best["x1"], best["y1"], best["x2"], best["y2"]]):
                        tracked[i] += TRACK_ALPHA * (value - tracked[i])
                lost_frames = 0
            else:
                lost_frames += 1
            ...
            if primary:
                ...
                if locked:
                    ...
                    if abs(dx) > DEADZONE_PX:
                        ...
                        speed = PAN_DIRECTION * (1 if dx > 0 else -1) * mag
                        pan1.start(speed=speed)
                        pan2.start(speed=speed)

                    # Tilt - pulse step
                    if abs(dy) > DEADZONE_PX:
                        mag = int(max(TILT_MIN_SPEED, min(TILT_MAX_SPEED, abs(dy) * TILT_KP)))
                        tilt_speed = TILT_DIRECTION * (1 if dy > 0 else -1) * mag
                        tilt.start(speed=tilt_speed)
                    ...
                    pan_dir = "R" if dx > 0 else "L"
                    tilt_dir = "D" if dy > 0 else "U"
                    print(
                        f"[AIM]  pan={pan_dir} tilt={tilt_dir}  err=({dx:+4d},{dy:+4d})"
                        f"  t_spd={tilt_speed:+4d}  {primary['conf'] * 100:.0f}%"
                    )
            else:
                if target_visible:
                    print("[LOST]")
...
def main():
    thread = threading.Thread(target=turret_loop, daemon=True)
    thread.start()
